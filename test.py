...
for Filename in filenames:
    ...
    if MaxZ: Z_Selected, ForceSelected = func.minforce(Z_Selected, ForceSelected,
                                                       -Lc * DNAds * 1.1)  # remove data above Z=1.1*LC
    ...
    ProbSum = func.probsum(ForceSelected, Z_Selected, Lmin, Lmax, Lc, p, S, Z_fiber,
                           k)  # Calculate probability landscape
    PeakInd, Peak = func.findpeaks(ProbSum, 25)  # Find Pesk
    Peaks = signal.find_peaks_cwt(ProbSum, np.arange(2.5, 30), max_distances=np.linspace(75, 75, len(
        ProbSum)))  # numpy peakfinder, finds too many peaks, not used plot anyway
    ...
    MergeStates = False
    if len(States) > 1: MergeStates = True
    P_Cutoff = 0.1  # Significance for merging states

    # Calculate for each datapoint which state it most likely belongs too
    Ratio = func.ratio(Lmin, Lmax, States)
    ZState = np.array(np.multiply(func.wlc(ForceSelected, p, S).reshape(len(func.wlc(ForceSelected, p, S)), 1),
                                  (States * DNAds)) + np.multiply(
        func.hook(ForceSelected, k, Fmax_Hook).reshape(len(func.hook(ForceSelected, k, Fmax_Hook)), 1),
        (Ratio * Z_fiber)))
    ...
    while MergeStates == True:  # remove states untill all states are significantly different
        ...
        for i, x in enumerate(States):
            if i > 0:
                Prob = stats.ttest_ind((StateMask == i) * Z_Selected, (StateMask == i - 1) * Z_Selected,
                                       equal_var=False)  # get two arrays for t_test
